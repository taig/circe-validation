...
trait ValidatingDecoder[A] extends Decoder[A] {
  ...
  override def decodeAccumulating(cursor: HCursor): AccumulatingDecoder.Result[A] =
...
object ValidatingDecoder {
  def instance[A](f: HCursor => AccumulatingDecoder.Result[A]): ValidatingDecoder[A] =
    new ValidatingDecoder[A] {
      override def decodeValidating(cursor: HCursor): AccumulatingDecoder.Result[A] = f(cursor)
    }
}
